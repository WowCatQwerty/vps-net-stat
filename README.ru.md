...
Интерактивное меню `vns` намеренно выводит пункты меню обычным текстом без принудительной покраски — цвет текста подстраивается под тему терминала (тёмную или светлую), а не задаётся жёстко захардкоженным значением, которое могло бы стать нечитаемым на некоторых фонах.

Некоторые эмуляторы терминала (например, Termius) применяют собственную эвристическую подсветку синтаксиса к отдельным английским словам внутри обычного текста (например, «info» или «Export»), из-за чего отдельные слова могут отображаться другим цветом, даже если vps-net-stat не отправляет для них никаких цветовых кодов. Это особенность самого эмулятора терминала, а не баг vps-net-stat — если это мешает, проверьте настройки подсветки/темы своего терминала.
